<script>
  ...
  function moveCaretToNextWord() {
    if (wordIndex < words.length) {
      ...
      letterIndex = 0;
      caretEl.style.top = `${nextWordEl.children[letterIndex].offsetTop + 4}px`;
  ...
  <div class="layout">
      ...
      {#if auth}
        <div class="nav-link">
          ...
          <button on:click={logout}
            ><i class="fa-solid fa-right-to-bracket icon-flipped" /></button
          >
        </div>
      {:else}
        <div class="nav-link">
          ...
          <a href="/login"><i class="fa-solid fa-right-to-bracket" /></a>
        </div>
      {/if}
    </nav>
    <main>
    ...
    <div class="social-media">
      <div class="social-icon">
        <a href="https://github.com/firzalay/type-master" target="_blank"
          >Discord</a
